...
parse_jump_location(String_Const_u8 line){
  ...
  if (!jump.success)
  {
    // NOTE(fakhri): odin style
    left_paren_pos = string_find_first(line, '(');
    right_paren_pos = left_paren_pos + string_find_first(string_skip(line, left_paren_pos), ')');
    ...
    if (file_name.size > 0)
    {
      ...
      u64 colone_pos = string_find_first(number, ':');
      if (colone_pos < number.size){
        String_Const_u8 line_number = string_prefix(number, colone_pos);
        String_Const_u8 column_number = string_skip(number, colone_pos + 1);
        if (string_is_integer(line_number, 10) && string_is_integer(column_number, 10)){
          jump.location.file = file_name;
          jump.location.line = (i32)string_to_integer(line_number, 10);
          jump.location.column = (i32)string_to_integer(column_number, 10);
          jump.colon_position = (i32)right_paren_pos;
          jump.success = true;
        }
      }
    }
