...
find_anchor_token(Application_Links *app, Buffer_ID buffer, Token_Array *tokens, i64 invalid_line){
    ...
    if (tokens != 0 && tokens->tokens != 0){
        ...
        for (;;token += 1){
            ...
            if (true || !HasFlag(token->flags,  TokenBaseFlag_PreprocessorBody)){
        ...
        for (;;){
            ...
            if (HasFlag(token->flags, TokenBaseFlag_PreprocessorBody)){
                ...
                switch (token->kind){
                    ...
                    case TokenBaseKind_ParentheticalClose:
                    {
                        for (;nest != 0 && nest->kind != TokenBaseKind_ParentheticalOpen;){
                            Nest *n = nest;
                            sll_stack_pop(nest);
                            indent__free_nest(&nest_alloc, n);
                        }
...
CUSTOM_DOC("Audo-indents the entire current buffer.")
{
    ...
    auto_indent_buffer(app, buffer, Ii64(0, buffer_size), Indent_ClearLine);
}
