...
internal F4_LANGUAGE_INDEXFILE(F4_CPP_IndexFile)
{
  ...
  for(b32 handled = 0; !ctx->done;)
  {
    ...
    //~ NOTE(fakhri): Namespaces
    else if (F4_Index_ParsePattern(ctx, "%t%t%k", "using", "namespace", TokenBaseKind_Identifier, &name))
    {
      handled = 1;
    }
    else if (F4_Index_ParsePattern(ctx, "%t%k", "namespace", TokenBaseKind_Identifier, &name))
    {
      handled = 1;
      F4_Index_MakeNote(ctx, Ii64(name), F4_Index_NoteKind_Namespace, 0);
    }
    ...
    //~NOTE(fakhri): Macro'd Typedef
    else if (F4_Index_ParsePattern(ctx, "%t%n%t%k", "typedef", F4_Index_NoteKind_Macro, &macro_name, "(",
        TokenBaseKind_Identifier, &name))
    {
      handled = 1;
    ...
    //~ NOTE(fakhri): 
    else if (F4_Index_ParsePattern(ctx, "%k%o%n%t%k%t",
				TokenBaseKind_Identifier, &base_type,
    ...
    //~ NOTE(rjf): Functions
    else if((F4_Index_ParsePattern(ctx, "%k%o%k%t",
          TokenBaseKind_Identifier, &base_type,
          TokenBaseKind_Identifier, &name,
          "(") ||
        F4_Index_ParsePattern(ctx, "%k%o%k%t",
    ...
    else if(F4_CPP_ParseDecl(ctx, &name))
    {
      handled = 1;
