...
VIM_COMMAND_SIG(vim_submode_s){ vim_state.sub_mode = SUB_S; vim_state.chord_resolved = false; }
...
VIM_COMMAND_SIG(vim_list_all_locations){
    vim_push_jump(app, get_active_view(app, Access_ReadVisible));
    list_all_locations(app);
}

VIM_COMMAND_SIG(vim_list_all_locations_of_identifier){
    vim_push_jump(app, get_active_view(app, Access_ReadVisible));
    list_all_locations_of_identifier(app);
}

VIM_COMMAND_SIG(vim_list_all_functions_all_buffers){
    vim_push_jump(app, get_active_view(app, Access_ReadVisible));
    list_all_functions_all_buffers(app);
}

VIM_COMMAND_SIG(vim_list_all_locations_of_selection){
    vim_push_jump(app, get_active_view(app, Access_ReadVisible));
...
VIM_COMMAND_SIG(vim_goto_definition_same_panel){
    vim_push_jump(app, get_active_view(app, Access_ReadVisible));
...
CUSTOM_DOC("Vim: Saves all the buffer") { save_all_dirty_buffers(app); }
...
CUSTOM_DOC("Vim: Open file") { interactive_open_or_new(app); }

CUSTOM_COMMAND_SIG(b)
CUSTOM_DOC("Vim: Open file") { interactive_switch_buffer(app); }
...
CUSTOM_DOC("Vim: kill the current buffer.") {
    kill_buffer(app);
}
