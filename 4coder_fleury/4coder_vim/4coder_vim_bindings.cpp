...
#define MAP 0
    ...
    VimBind(N|V|MAP, set_mark,                             KeyCode_Space);
    ...
    VimBind(N|MAP, swap_panels,                       (Ctl|KeyCode_2));
    ...
    VimBind(N|V|MAP, vim_submode_s,                        KeyCode_S);
    ...
    VimBind(N|V|MAP, vim_select_register,             (Ctl|KeyCode_Quote));
    ...
    /// Movement Binds
    VimBind(N|V|MAP, vim_left,                             KeyCode_J);
    VimBind(N|V|MAP, vim_down,                             KeyCode_K);
    VimBind(N|V|MAP, vim_up,                               KeyCode_L);
    VimBind(N|V|MAP, vim_right,                            KeyCode_Semicolon);
    VimBind(N|V|MAP, vim_end_line,                    (Sft|KeyCode_Semicolon));
    VimBind(N|V|MAP, vim_begin_line,                  (Sft|KeyCode_J));
    ...
    VimBind(N|V|MAP, vim_goto_definition,             SUB_G,     KeyCode_D);
    VimBind(N|V|MAP, vim_goto_definition_same_panel,  SUB_G, Sft|KeyCode_D);
    ...
    VimBind(N|V|MAP, vim_set_seek_char,               (Sft|KeyCode_3));
    VimBind(N|V|MAP, vim_seek_char_forward,       (Ctl|Sft|KeyCode_Semicolon));
    VimBind(N|V|MAP, vim_seek_char_backward,      (Ctl|Sft|KeyCode_J));
    VimBind(N|V|MAP, vim_paragraph_up,                (Sft|KeyCode_L));
    VimBind(N|V|MAP, vim_paragraph_down,              (Sft|KeyCode_K));
#if 0
    ...
    VimBind(N|V|MAP, cursor_mark_swap,                (Sft|KeyCode_Space));
    ...
    /// Screen Adjust Binds
    VimBind(N|V|MAP, vim_half_page_up,            (Ctl|Sft|KeyCode_B));
    VimBind(N|V|MAP, vim_half_page_down,          (Ctl|Sft|KeyCode_F));
    ...
    VimBind(N|V|MAP, casey_find_matching_file,               KeyCode_M);
    
    VimBind(N|V|MAP, vim_goto_mark,                        KeyCode_Tick);
    VimBind(N|V|MAP, vim_goto_mark,                        KeyCode_Quote);
    
    VimBind(N|V|MAP, vim_toggle_macro,                (Ctl|KeyCode_U));
    VimBind(N|V|MAP, vim_play_macro,                  (Alt|KeyCode_U));
    
    /// Window Binds (TODO(BYP): Do the rest of these, and do them properly)
    VimBind(N|MAP, change_active_panel_backwards,     (Ctl|KeyCode_J));
    VimBind(N|MAP, change_active_panel_backwards,     (Ctl|KeyCode_K));
    VimBind(N|MAP, change_active_panel,               (Ctl|KeyCode_L));
    VimBind(N|MAP, change_active_panel,               (Ctl|KeyCode_Semicolon));
    VimBind(N|MAP, change_active_panel,                    KeyCode_Tab);
    VimBind(N|MAP, swap_panels,                       (Sft|KeyCode_Tab));
    
#if 0    
    VimBind(N|MAP, TAB_next,                           (Ctl|KeyCode_Tab));
    VimBind(N|MAP, TAB_prev,                       (Sft|Ctl|KeyCode_Tab));
    VimBind(N|MAP, TAB_new,                            (Ctl|KeyCode_T));
    VimBind(N|MAP, TAB_close,                      (Sft|Ctl|KeyCode_W));
#endif
    
    VimBind(N|MAP,   vim_list_all_locations, SUB_S, KeyCode_L);
    VimBind(N|V|MAP, vim_list_all_locations_of_identifier, SUB_S, KeyCode_I);
    VimBind(N|V|MAP, vim_list_all_functions_all_buffers,    SUB_S, KeyCode_F);
    VimBind(N|V|MAP, vim_list_all_locations_of_selection,  SUB_S, KeyCode_R);
    
    VimBind(N|V|MAP, f4_toggle_enclosure_side,  KeyCode_H);
    
    VimBind(N|V|MAP, f4_search_for_definition__project_wide,  (Alt|Ctl|KeyCode_J));
    
  VimBind(N|MAP, f4_toggle_compilation_expand,             KeyCode_Q);
