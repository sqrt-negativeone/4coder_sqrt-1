    ...
    else
    {
        ...
        else
        {
            ...
            switch(vim_state.mode){
                case VIM_Normal:        color = F4_ARGBFromID(table, modal_cursor_normal);  break;
                case VIM_Insert:        color = F4_ARGBFromID(table, modal_cursor_insert);  break;
                case VIM_Replace:       color = F4_ARGBFromID(table, modal_cursor_change);  break;
            }
            
            switch(vim_state.sub_mode)
            {
                case SUB_G:      color = F4_ARGBFromID(table, modal_cursor_goto); break;
                case SUB_S:      color = F4_ARGBFromID(table, modal_cursor_search); break;
            }
            
            switch(vim_state.params.request)
            {
                case REQUEST_Yank:   color = F4_ARGBFromID(table, modal_cursor_yank); break;
                case REQUEST_Delete: color = F4_ARGBFromID(table, modal_cursor_delete); break; 
                case REQUEST_Change: color = F4_ARGBFromID(table, modal_cursor_change); break;
            }
        }
    }
